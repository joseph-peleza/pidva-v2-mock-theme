...
import {allNews, bitCoinNews, lightCoinNews, rippleNews} from "../../../routes/main/dashboard/DashOne/data"
import CircularProgress from "components/CircularProgress";
import DashOneNewsItem from "./DashOneNewsItem";

const newsData = [allNews, bitCoinNews, lightCoinNews, rippleNews];

class DashOneNews extends React.Component {
  state = {
    news: newsData[0],
    loader: false,
  };
    ...
    return (
      <Widget>
        <div className="ant-row-flex gx-justify-content-between gx-mb-3 gx-dash-search">
          <h2 className="h4 gx-mb-3 gx-mb-sm-1 gx-mr-2">DashOne News</h2>
          <div className="gx-mx-sm-2">
            <Radio.Group className="gx-radio-group-link gx-radio-group-link-news" defaultValue={0}
              ...
              <Radio.Button value={1} className="gx-mb-1">Verifications</Radio.Button>
        ...
        {loader ? <CircularProgress className="gx-loader-400"/> : news.map((data, index) =>
          <DashOneNewsItem key={index} data={data}/>
        )}
